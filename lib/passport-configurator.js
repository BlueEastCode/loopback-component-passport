...
//   this.userModel = options.userModel || loopback.getModelByType(this.app.models.User);
  ...
  if (!this.accountModel.relations.identities) {
    this.accountModel.hasMany(this.userIdentityModel, {as: 'identities'});
  } else {
    this.userIdentityModel = this.accountModel.relations.identities.modelTo;
  }

  if (!this.accountModel.relations.credentials) {
    this.accountModel.hasMany(this.userCredentialModel, {as: 'credentials'});
  } else {
    this.userCredentialModel = this.accountModel.relations.credentials.modelTo;
  }

  if (!this.userIdentityModel.relations.user) {
    this.userIdentityModel.belongsTo(this.accountModel, {as: 'account'});
  }

  if (!this.userCredentialModel.relations.user) {
    this.userCredentialModel.belongsTo(this.accountModel, {as: 'account'});
  }
};
...
PassportConfigurator.prototype.init = function(noSession) {
  ...
  if (!noSession) {
    ...
    passport.deserializeUser(function(id, done) {
      ...
      self.accountModel.findById(id, function(err, user) {
        if (err || !user) {
...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var failureRedirect = options.failureRedirect ||
    (link ? '/link.html' : config.google.successRedirect);
  ...
  switch (authType) {
    ...
    case 'local':
      ...
      function(username, password, done) {
        ...
        self.accountModel.findOne(query, function(err, user) {
          if (err)
            return done(err);
        ...
        req.logIn(user, function(err) {
          ...
          if (info && info.accessToken) {
            if (!!options.json) {
              ...
              foo.put('access_token', info.accessToken.id);
              foo.put('userId', user.id.toString());
            }
          }
